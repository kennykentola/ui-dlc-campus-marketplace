...
const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-100 transition-all duration-500 ${isScrolled ? "bg-white/95 backdrop-blur-2xl border-b border-slate-100 py-3 shadow-md" : "bg-white/70 backdrop-blur-xl py-5"}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-10 max-w-[1600px] flex items-center justify-between gap-4">
        ...
        <div className="flex items-center shrink-0">
           <Link to="/" className="flex items-center gap-4 group lg:mr-10">
              <div className="w-11 h-11 bg-linear-to-br from-brand-primary to-[#312E81] rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-950/20 group-hover:scale-105 transition-transform duration-500">
              ...
              <div className="hidden sm:block">
                 <h1 className="text-xl font-black text-brand-primary tracking-[0.08em] uppercase leading-none">
                    UI DLC <span className="text-[#312E81]">HUB.</span>
                 </h1>
        ...
        <nav className="hidden lg:flex grow items-center justify-center gap-3 rounded-full border border-slate-200/80 bg-white/80 px-4 py-3 text-[11px] font-bold uppercase tracking-[0.24em] text-slate-500 shadow-sm">
             ...
             <Link 
               key={link.name}
               to={link.path} 
               className={`rounded-full px-5 py-2.5 transition-all relative ${location.pathname === link.path ? "text-brand-primary bg-brand-surface" : "hover:bg-slate-50 hover:text-brand-primary"}`}
             >
               {link.name}
        ...
        <div className="flex items-center gap-3 sm:gap-4 shrink-0 lg:ml-8">
           {user ? (
              <div className="flex items-center gap-3 bg-white/85 border border-slate-200 pl-3 pr-4 py-2 rounded-2xl group cursor-pointer hover:border-brand-primary/30 transition-all shadow-sm">
                 <Link to="/profile" className="flex items-center gap-3">
                    ...
                    <div className="hidden md:block text-left">
                       ...
                       <p className="text-[9px] font-bold uppercase text-brand-primary tracking-[0.22em] mt-1 italic">Student</p>
                    </div>
                 </Link>
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                 ...
                 <Link to="/register" className="px-6 py-3 bg-brand-primary text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.22em] hover:brightness-110 shadow-xl shadow-brand-primary/10 transition-all active:scale-95">Register</Link>
              </div>
           )}

           <Link to="/sell" className="hidden lg:flex items-center gap-3 px-6 py-3 bg-brand-primary text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.22em] hover:brightness-110 shadow-2xl shadow-indigo-800/20 transition-all border border-brand-primary active:scale-95">
      ...
      <div className={`fixed inset-0 z-101 lg:hidden transition-all duration-500 ${isMobileMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
         ...
         <div className={`absolute right-0 top-0 h-full w-[310px] bg-white shadow-2xl transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] flex flex-col p-8 ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
            ...
            <div className="flex items-center justify-between mb-12">
               ...
               <button onClick={() => setIsMobileMenuOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-brand-primary hover:bg-slate-100 transition-all">
            ...
            <div className="flex flex-col gap-3">
               {navLinks.map((link) => (
                 <Link 
                    key={link.name}
                    to={link.path}
                    className={`flex items-center justify-between px-6 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all border ${location.pathname === link.path ? "bg-brand-primary text-white border-brand-primary shadow-xl shadow-brand-primary/20" : "bg-slate-50 text-slate-500 border-slate-100 hover:bg-slate-100"}`}
                 >
                    {link.name}
                    {link.hasNotification && <span className={`w-2 h-2 rounded-full ${location.pathname === link.path ? 'bg-white' : 'bg-brand-primary'}`}></span>}
                 </Link>
               ))}

               <Link 
                  to="/sell"
                  className="mt-6 flex items-center justify-center gap-3 px-6 py-5 bg-brand-primary text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-2xl shadow-brand-primary/20 active:scale-95 transition-all"
               >
                  List Asset <i className="fa-solid fa-plus-circle"></i>
               </Link>
            </div>
            ...
            <div className="mt-auto pt-8 border-t border-slate-100">
               {user ? (
                 <div className="space-y-6">
                    <div className="flex items-center gap-4 px-4 py-4 bg-slate-50 rounded-[28px] border border-slate-100">
                       ...
                       <div>
                          ...
                          <p className="text-[10px] font-bold uppercase text-brand-primary tracking-widest mt-1 italic leading-none">Student</p>
                       </div>
                    </div>
                    <button 
                      onClick={logout}
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    ...
                    <Link to="/register" className="flex items-center justify-center py-4 bg-brand-primary text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-brand-primary/10">Register</Link>
                 </div>
               )}
            </div>
         </div>
