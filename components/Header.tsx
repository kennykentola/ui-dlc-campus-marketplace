  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-100 transition-all duration-500 ${isScrolled ? "bg-white/95 backdrop-blur-2xl border-b border-slate-100 py-3 shadow-md" : "bg-white/70 backdrop-blur-xl py-5"}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-10 max-w-[1600px] flex items-center justify-between gap-4">
           ...
           <Link to="/" className="flex items-center gap-4 group lg:mr-10">
              ...
              <div className="hidden sm:block">
                 <h1 className="text-xl font-black text-[#003366] tracking-[0.08em] uppercase leading-none">
                    UI DLC <span className="text-[#0f766e]">HUB.</span>
                 </h1>
        ...
        <nav className="hidden lg:flex grow items-center justify-center gap-3 rounded-full border border-slate-200/80 bg-white/80 px-4 py-3 text-[11px] font-bold uppercase tracking-[0.24em] text-slate-500 shadow-sm">
             ...
             <Link 
               key={link.name}
               to={link.path} 
               className={`rounded-full px-5 py-2.5 transition-all relative ${location.pathname === link.path ? "text-[#003366] bg-teal-50" : "hover:bg-slate-50 hover:text-[#003366]"}`}
             >
               {link.name}
               {link.hasNotification && (
                 <span className="absolute top-2 right-2 w-2 h-2 bg-teal-600 rounded-full"></span>
               )}
             </Link>
           ))}
        </nav>
        ...
        <div className="flex items-center gap-3 sm:gap-4 shrink-0 lg:ml-8">
           {user ? (
              <div className="flex items-center gap-3 bg-white/85 border border-slate-200 pl-3 pr-4 py-2 rounded-2xl group cursor-pointer hover:border-teal-600/30 transition-all shadow-sm">
           ...
           <Link to="/sell" className="hidden lg:flex items-center gap-3 px-6 py-3 bg-teal-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.22em] hover:brightness-110 shadow-2xl shadow-teal-700/20 transition-all border border-teal-600 active:scale-95">
           ...
           <button 
             ...
             className="lg:hidden w-11 h-11 flex items-center justify-center rounded-2xl border border-slate-200 bg-white shadow-sm active:scale-95 transition-all text-[#003366]"
           >
      ...
      <div className={`fixed inset-0 z-101 lg:hidden transition-all duration-500 ${isMobileMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
         <div className="absolute inset-0 bg-[#003366]/40 backdrop-blur-md" onClick={() => setIsMobileMenuOpen(false)}></div>
         <div className={`absolute right-0 top-0 h-full w-[310px] bg-white shadow-2xl transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] flex flex-col p-8 ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
            ...
            <div className="flex items-center justify-between mb-12">
               ...
               <button onClick={() => setIsMobileMenuOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-[#003366] hover:bg-slate-100 transition-all">
            ...
            <div className="flex flex-col gap-3">
               {navLinks.map((link) => (
                 <Link 
                    key={link.name}
                    to={link.path}
                    className={`flex items-center justify-between px-6 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all border ${location.pathname === link.path ? "bg-teal-600 text-white border-teal-600 shadow-xl shadow-teal-500/20" : "bg-slate-50 text-slate-500 border-slate-100 hover:bg-slate-100"}`}
                 >
                    {link.name}
                    {link.hasNotification && <span className={`w-2 h-2 rounded-full ${location.pathname === link.path ? 'bg-white' : 'bg-teal-600'}`}></span>}
                 </Link>
               ))}

               <Link 
                  to="/sell"
                  className="mt-6 flex items-center justify-center gap-3 px-6 py-5 bg-[#003366] text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-2xl shadow-blue-900/20 active:scale-95 transition-all"
               >
                  List Asset <i className="fa-solid fa-plus-circle"></i>
               </Link>
            </div>
