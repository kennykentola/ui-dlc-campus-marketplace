...
file_path = 'c:/Users/user/Downloads/ui-dlc-campus-marketplace/pages/AdminDashboard.tsx'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

target = 'if (user?.role !== UserRole.ADMIN) return null;'
        ...
        <aside className=\"w-64 bg-[#004080] text-white hidden lg:flex flex-col py-8 shadow-xl z-10 shrink-0\">
          <nav className=\"flex-1 space-y-2 px-4\">
            <button 
              ...
              className={w-full flex items-center gap-4 px-5 py-4 rounded-xl text-sm font-medium transition-all \}
            >
                    ...
                    <div className=\"fixed inset-0 z-[100] flex items-center justify-center p-4\">
                       ...
                       <div className=\"relative z-10 w-full max-w-2xl rounded-2xl bg-white p-8 shadow-2xl\">
                          <div className=\"flex items-start justify-between gap-4 border-b border-slate-100 pb-6 mb-6\">
                             <div className=\"flex items-center gap-4\">
                                <img
                                  src={selectedUser.avatarUrl || \https://ui-avatars.com/api/?name=\&background=003366&color=fff\}
                  ...
                  {activeTab === 'network' && (
                     <div className=\"grid grid-cols-1 lg:grid-cols-3 gap-6\">
                        <div className=\"lg:col-span-2 space-y-6\">
                                    ...
                                    <tbody className=\"divide-y divide-slate-50\">
                                       {users.slice(0, 3).map((u, i) => (
                                          <tr key={i} className=\"text-slate-700\">
                                             ...
                                             <td className=\"py-3 font-medium text-[#003366]\">{u.name}</td>
                                             <td className=\"py-3\">Account Created</td>
                                             <td className=\"py-3\"><span className=\"text-xs bg-green-100 text-green-700 px-2 py-1 rounded\">Success</span></td>
                                          </tr>
                                       ))}
                                       {products.slice(0, 3).map((p, i) => (
                                          <tr key={\p-\\} className=\"text-slate-700\">
                                             <td className=\"py-3 text-xs text-slate-400\">{new Date(p.createdAt).toLocaleDateString()}</td>
                                             <td className=\"py-3 font-medium text-[#003366]\">Seller {p.sellerId.substring(0,4)}</td>
                                             <td className=\"py-3\">Listed Asset: {p.name}</td>
                                             <td className=\"py-3\"><span className=\"text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded\">Active</span></td>
                                          </tr>
                                       ))}
                                    </tbody>
                                 </table>
                           ...
                           <div className=\"bg-white p-6 rounded-2xl shadow-sm border border-slate-200\">
                              <h3 className=\"text-lg font-bold text-slate-700 mb-4 border-b border-slate-100 pb-2\">Recent Listings Overview</h3>
                              <div className=\"overflow-x-auto\">
                                 <table className=\"w-full text-left text-sm\">
                                    <thead>
                                       <tr className=\"text-slate-500 border-b border-slate-100\">
                                    ...
                                    <tbody className=\"divide-y divide-slate-50\">
                                       {products.slice(0, 5).map(p => (
                                          <tr key={p.} className=\"text-slate-700\">
                                             <td className=\"py-3 font-medium text-[#003366] truncate max-w-[150px]\">{p.name}</td>
                                             <td className=\"py-3\">?{p.price.toLocaleString()}</td>
                                             <td className=\"py-3 text-xs font-mono text-slate-400\">{p.sellerId.substring(0,8)}</td>
                                             <td className=\"py-3 text-right\">
                                                <Link to={\/product/\\} className=\"px-3 py-1 bg-slate-100 text-[#003366] rounded hover:bg-slate-200 text-xs font-medium\">View</Link>
                                             </td>
                                          </tr>
                                       ))}
                                    </tbody>
                        ...
                        <div className=\"space-y-6\">
                           <div className=\"bg-white p-6 rounded-2xl shadow-sm border border-slate-200\">
                              <h3 className=\"text-sm font-bold text-slate-700 mb-4\">System Statistics</h3>
                              <div className=\"h-40 w-full flex items-end justify-between gap-2 border-b border-l border-slate-200 p-2 relative\">
                                 {/* CSS Bar Chart Simulation */}
                                 <div className=\"w-1/5 bg-blue-400/50 hover:bg-blue-500 transition-colors h-[30%] rounded-t-sm relative group\"><span className=\"absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] opacity-0 group-hover:opacity-100\">30%</span></div>
                                 <div className=\"w-1/5 bg-blue-400/50 hover:bg-blue-500 transition-colors h-[50%] rounded-t-sm relative group\"><span className=\"absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] opacity-0 group-hover:opacity-100\">50%</span></div>
                                 <div className=\"w-1/5 bg-blue-400/50 hover:bg-blue-500 transition-colors h-[40%] rounded-t-sm relative group\"><span className=\"absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] opacity-0 group-hover:opacity-100\">40%</span></div>
                                 <div className=\"w-1/5 bg-blue-400/50 hover:bg-blue-500 transition-colors h-[80%] rounded-t-sm relative group\"><span className=\"absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] opacity-0 group-hover:opacity-100\">80%</span></div>
                                 <div className=\"w-1/5 bg-blue-400/50 hover:bg-blue-500 transition-colors h-[60%] rounded-t-sm relative group\"><span className=\"absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] opacity-0 group-hover:opacity-100\">60%</span></div>
                              </div>
                           </div>
                           
                           <div className=\"bg-white p-6 rounded-2xl shadow-sm border border-slate-200\">
                              <h3 className=\"text-sm font-bold text-slate-700 mb-4\">New Users</h3>
                              <div className=\"h-40 w-full flex items-end justify-between gap-2 border-b border-l border-slate-200 p-2 relative\">
                                 <div className=\"w-1/4 bg-green-400/50 hover:bg-green-500 transition-colors h-[20%] rounded-t-sm\"></div>
                                 <div className=\"w-1/4 bg-green-400/50 hover:bg-green-500 transition-colors h-[60%] rounded-t-sm\"></div>
                                 <div className=\"w-1/4 bg-green-400/50 hover:bg-green-500 transition-colors h-[45%] rounded-t-sm\"></div>
                                 <div className=\"w-1/4 bg-green-400/50 hover:bg-green-500 transition-colors h-[90%] rounded-t-sm\"></div>
                              </div>
                           </div>
                        </div>
                     </div>
                                 ...
                                 {products.map(p => (
                                    <tr key={p.} className=\"hover:bg-slate-50/50 transition-colors\">
                                       <td className=\"py-4\">
                                          <div className=\"flex items-center gap-4\">
                                             ...
                                             <div>
                                                ...
                                                <p className=\"text-xs text-slate-400 font-mono\">ID: {p..substring(0,8)}</p>
                                             </div>
                                          </div>
                                       </td>
                                       <td className=\"py-4\">
                                       ...
                                       <td className=\"py-4 text-right space-x-2\">
                                          <Link to={\/product/\\} className=\"px-3 py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold hover:bg-blue-100 transition-colors inline-block\">Inspect</Link>
                                          <button onClick={() => deleteProduct(p.)} className=\"px-3 py-2 bg-rose-50 text-rose-600 rounded-lg text-xs font-bold hover:bg-rose-100 transition-colors inline-block\">Delete</button>
                                       </td>
                                    </tr>
                                 ))}
                              ...
                              <tbody className=\"divide-y divide-slate-50\">
                                 {users.map(u => (
                                    <tr key={u.userId} className=\"hover:bg-slate-50/50 transition-colors\">
                                       <td className=\"py-4\">
                                          <div className=\"flex items-center gap-4\">
                                             <img src={u.avatarUrl || \https://ui-avatars.com/api/?name=\&background=003366&color=fff\} alt=\"\" className=\"w-10 h-10 rounded-lg shadow-sm\" />
                                             <div>
                                       ...
                                       <td className=\"py-4\">
                                          <span className={px-3 py-1 rounded-md text-xs font-bold \}>{u.sellerStatus}</span>
                                       </td>
                                       <td className=\"py-4 text-right space-x-2\">
                  ...
                  {activeTab === 'reports' && (
                     <div className=\"bg-white p-6 rounded-2xl shadow-sm border border-slate-200\">
                        ...
                        <div className=\"space-y-4\">
                           {reports.map(r => (
                              <div key={r.} className={p-5 rounded-xl border \}>
                                 <div className=\"flex flex-col md:flex-row justify-between gap-4\">
                                    <div>
                                       <div className=\"flex items-center gap-3 mb-2\">
                                          <h4 className=\"text-base font-bold text-[#003366]\">{r.reason}</h4>
                                          <span className={	ext-xs px-2 py-0.5 rounded font-bold \}>{r.status || 'Pending'}</span>
                                       </div>
                                    ...
                                    {r.status !== 'resolved' && (
                                       <div className=\"flex md:flex-col gap-2 shrink-0\">
                                          <button onClick={() => resolveReport(r., 'investigating' as any)} disabled={r.status === 'investigating' || resolvingReportId === r.} className=\"px-4 py-2 bg-amber-100 text-amber-700 text-xs font-bold rounded-lg hover:bg-amber-200 disabled:opacity-50\">Investigate</button>
                                          <button onClick={() => resolveReport(r., 'resolved')} disabled={resolvingReportId === r.} className=\"px-4 py-2 bg-[#003366] text-white text-xs font-bold rounded-lg hover:bg-[#004080] disabled:opacity-50\">Resolve</button>
                                       </div>
                                    )}
                                 </div>
                              </div>
