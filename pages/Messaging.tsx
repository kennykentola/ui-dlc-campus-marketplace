      ...
      <div className={`w-full md:w-1/3 lg:w-[450px] bg-slate-50 border-r border-slate-100 flex flex-col h-full ${chattingWith ? "hidden md:flex" : "flex"}`}>
        ...
        <div className="grow overflow-y-auto bg-white no-scrollbar">
          {conversations.map((c, i) => (
            <button key={i} onClick={() => { setChattingWith(c.user); navigate(`/messages?with=${c.user.userId}`); }} className={`w-full h-20 md:h-24 px-6 md:px-10 flex items-center gap-4 md:gap-6 border-b border-slate-50 transition-all ${chattingWith?.userId === c.user.userId ? "bg-brand-surface" : "hover:bg-slate-50"}`}>
              ...
              <div className="grow text-left overflow-hidden">
                <div className="flex justify-between items-center mb-1">
                  <p className="text-[13px] md:text-[15px] font-black text-brand-primary uppercase tracking-tight truncate leading-none">{c.user.name}</p>
      ...
      <div className={`grow flex flex-col h-full bg-white relative ${!chattingWith ? "hidden md:flex" : "flex"}`}>
        {chattingWith ? (
          <>
            <div className="h-20 md:h-24 px-4 md:px-12 flex items-center justify-between bg-white/80 backdrop-blur-xl border-b border-slate-100 z-60 shrink-0">
              <div className="flex items-center gap-3 md:gap-6">
                <button onClick={() => setChattingWith(null)} className="md:hidden w-10 h-10 flex items-center justify-center rounded-xl bg-slate-100 text-brand-primary"><i className="fa-solid fa-arrow-left"></i></button>
              ...
              <div className="flex gap-2 md:gap-4 relative z-50">
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {WALLPAPERS.map(wp => {
                        ...
                        return (
                          ...
                          >
                            <div 
                              className={`w-12 h-12 rounded-2xl border-2 ${chatWallpaper === wp.id ? 'border-brand-primary scale-110' : 'border-slate-100'} shadow-sm hover:scale-110 transition-transform flex items-center justify-center overflow-hidden`}
                              style={{ 
                ...
                <button onClick={initiateCall} className="w-10 h-10 md:w-14 md:h-14 flex items-center justify-center bg-brand-surface text-brand-primary rounded-xl md:rounded-2xl hover:bg-brand-primary hover:text-white transition-all shadow-sm active:scale-95 group">
                        ...
                        <div className={`px-4 py-3 md:px-8 md:py-6 rounded-[24px] md:rounded-[32px] text-[13px] md:text-[15px] font-medium leading-relaxed border wrap-break-word w-full shadow-sm ${isMe ? "bg-brand-primary text-white rounded-tr-none border-brand-primary text-right" : "bg-white text-brand-ink rounded-tl-none border-slate-50 text-left"}`}>
                          {m.type === 'file' ? (
                            ...
                            <div className="flex items-center gap-4 md:gap-6 py-1 md:py-2">
                              <button onClick={() => toggleAudio(m.$id)} className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all ${isMe ? 'bg-white text-brand-primary' : 'bg-brand-primary text-white'}`}>
                  ...
                  {isRecording ? (
                    ...
                    <div className="grow relative flex items-center">
                      <form onSubmit={sendMessage} className="grow">
                        ...
                        />
                        {newMessage.trim() && (
                          <button type="submit" className="absolute right-1.5 md:right-3 top-1/2 -translate-y-1/2 w-10 h-10 md:w-16 md:h-16 flex items-center justify-center rounded-xl md:rounded-[30px] bg-brand-primary text-white shadow-xl transition-all scale-105 active:scale-95">
                            <i className="fa-solid fa-paper-plane text-[10px] md:text-xl"></i>
                          </button>
                        )}
                      </form>
                      {!newMessage.trim() && (
                        <button onClick={startRecording} className="absolute right-1.5 md:right-3 top-1/2 -translate-y-1/2 w-10 h-10 md:w-16 md:h-16 flex items-center justify-center bg-brand-primary text-white rounded-xl md:rounded-[30px] hover:bg-brand-primary transition-all shadow-md active:scale-90">
          ...
          <div className="grow flex flex-col items-center justify-center p-10 md:p-20 text-center bg-slate-50/20">
            ...
            <h2 className="text-2xl md:text-4xl font-black text-brand-primary uppercase tracking-tighter">Hub Terminal.</h2>
