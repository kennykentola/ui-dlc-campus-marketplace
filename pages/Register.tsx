...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!matricRegex.test(formData.matricNumber)) {
      setError(
        "Invalid ID format. Use 'e0XXXXXX' (DLC) or 'XXXXXX' (Regular UI) e.g. e047307",
      );
      setLoading(false);
      return;
    }
    ...
    if (
      ...
      formData.adminCode !== ADMIN_SECRET
    ) {
      setError(
        "Incorrect Admin Access Code. Please contact the IT department.",
      );
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto py-6 md:py-8 px-4">
      <div className="bg-white p-6 md:p-12 rounded-4xl md:rounded-[40px] border border-slate-100 shadow-2xl shadow-slate-200">
        <div className="mb-10 text-center md:text-left">
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">
            Create Account
          </h1>
          <p className="text-slate-500 mt-2 font-medium">
            Join the UI DLC student & staff community.
          </p>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex bg-slate-50 p-1.5 rounded-2xl border border-slate-200">
              <button
                ...
                className={`flex-1 py-3.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${formData.role === UserRole.STUDENT ? "bg-blue-700 text-white shadow-lg" : "text-slate-400 hover:text-slate-600"}`}
              >
              ...
              <button
                ...
                className={`flex-1 py-3.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${formData.role === UserRole.ADMIN ? "bg-amber-500 text-white shadow-lg" : "text-slate-400 hover:text-slate-600"}`}
              >
